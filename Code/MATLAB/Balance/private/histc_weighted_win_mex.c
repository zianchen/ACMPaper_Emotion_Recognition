...
static char rcsid[] = "$Id: histc.c,v 1.9 2001/04/15 12:01:23 scott Exp $";
...
PRIVATE void _HistLoop(
    ...
    const mxArray *weights, /* G: weightings for input array */
    ...
    )
{
	...
	for (j=0; j < n2; j++)
	{
		...
		if (bin != NOBIN)
		  yr[bin*stride] = yr[bin*stride] + *wr; // G: modified
		  //yr[bin*stride]++; // G: modified
		if (pbin != NULL)
		{
...
PRIVATE void _NonDoubleHistLoop(
    const mxArray *x, /* input array */
    const mxArray *weights, /* G: weights for input array */
    const mxArray *edges, /* input array */
    mxArray *y,       /* output array */
    mxArray *bin_output, /* bin index output array */
    int stride,       /* stride along active dimension */
   ...
   )
{
    ...
    //G: added:
    weights = prhs[1];  
    if (mxIsEmpty(weights))
        mexErrMsgTxt("Second input must be non-empty numeric array.");
        
    edges = muMustBeDouble(prhs[2]);  // G: mod
	...
	if (mxGetClassID(x) == mxDOUBLE_CLASS)
	   _HistLoop(x,weights,edges,plhs[0],bin_output,stride,m,nbins); //G: modified, added weights
	else
	   _NonDoubleHistLoop(x,weights,edges,plhs[0],bin_output,stride,m,nbins); //G: modified, added weights
    }
    
    //if (edges != prhs[1]) 
    if (edges != prhs[2]) //G mod
